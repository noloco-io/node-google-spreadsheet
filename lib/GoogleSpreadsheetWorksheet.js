...
class GoogleSpreadsheetWorksheet {
  ...
  async getRows(options = {}) {
    ...
    const valuesOptions = Object.entries(options).reduce((acc, [key, value]) => ({
      ...acc,
      ...(['limit', 'offset'].includes(key) ? {} : { [key]: value })
    }), {})

    if (!this.headerValues) await this.loadHeaderRow(undefined, valuesOptions);
